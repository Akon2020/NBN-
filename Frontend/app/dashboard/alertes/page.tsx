...
export default function AlertesPage() {
  ...
  const [newSeverite, setNewSeverite] =
    useState<AlertSeverite>("AVERTISSEMENT");
  ...
  const handleAdvance = async (alert: Alert) => {
    ...
    try {
      ...
      setAlerts((prev) =>
        prev.map((a) => (a.idAlert === updated.idAlert ? updated : a)),
      );
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Suivi en direct - mises à jour automatiques via Socket.IO, avec
            repli REST
          </p>
        </div>
        <Button
              ...
              <CardContent className="p-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge variant="outline">
                      {ALERT_STATUT_LABELS[alert.statut]}
                    </Badge>
                  </div>
                  {alert.description && (
                    <p className="text-sm text-muted-foreground mt-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    ...
                    {alert.assignee
                      ? ` · Assignée à ${alert.assignee.fullName}`
                      : ""}
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  {alert.statut !== "CLOTUREE" && directory.length > 0 && (
                    <Select
                      value={
                        alert.assignee ? String(alert.assignee.idUser) : ""
                      }
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select
                  value={newSeverite}
                  onValueChange={(v) => setNewSeverite(v as AlertSeverite)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {Object.entries(ALERT_SEVERITE_LABELS).map(
                      ([value, label]) => (
