...
// BACK-G15 — catalogue RBAC pour le calcul et le pilotage des commissions.
const NEW_PERMISSIONS = [
  { key: "commissions:read", description: "Consulter les commissions" },
  {
    key: "commissions:manage",
    description: "Calculer une commission et la marquer due pour paiement",
  },
];

const ROLE_PERMISSIONS = {
  tresorerie: ["commissions:read", "commissions:manage"],
  operations: ["commissions:read", "commissions:manage"],
};
...
module.exports = {
  async up(queryInterface) {
    ...
    const [existing] = await queryInterface.sequelize.query(
      "SELECT `key` FROM permissions WHERE `key` = 'commissions:read'"
    );
    if (existing.length) {
